...
ospfs_read(struct file *filp, char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	while (amount < count && retval >= 0) {
		...
		if (n > count - amount) //Make sure we don't read more than asked
			n = count - amount;

		if (copy_to_user(buffer, data + offset, n) != 0) // copy from data into buffer
			return -EFAULT;
...
create_blank_direntry(ospfs_inode_t *dir_oi)
{
	...
	uint32_t offset, ret_val;

	//loop through entries to find an empty one
	for (offset = 0; offset < dir_oi->oi_size; offset += OSPFS_DIRENTRY_SIZE) 
	{
		new_entry = ospfs_inode_data(dir_oi, offset);

		if (new_entry->od_ino == 0) //
			return new_entry;
	}

	//If no empty entry is found, add a block to the directory
	ret_val = add_block(dir_oi);
...
ospfs_link(struct dentry *src_dentry, struct inode *dir, struct dentry *dst_dentry) {
	...
	//Make sure to null terminate the string as the null bit isn't copied over
...
static int
ospfs_create(struct inode *dir, struct dentry *dentry, 
	int mode, struct nameidata *nd)
{
		...
		for (entry_ino = 0; entry_ino < ospfs_super->os_ninodes; entry_ino++)
	{
		...
		if (file_oi != 0 && file_oi->oi_nlink == 0)
			break;
	}
	...
	/* Execute this code after your function has successfully created the
	   ...
	   getting here. */
	{
		struct inode *temp_ino = ospfs_mk_linux_inode(dir->i_sb, entry_ino);
		if (!temp_ino)
			return -ENOMEM;
		d_instantiate(dentry, temp_ino);
		return 0;
	}
}
...
ospfs_symlink(struct inode *dir, struct dentry *dentry, const char *symname)
{
	...
	uint32_t entry_ino = 0, ret_val;

	int symname_len = strlen(symname);


	//Check if name is too long
	if (dentry->d_name.len > OSPFS_MAXNAMELEN || symname_len > OSPFS_MAXNAMELEN)
		return -ENAMETOOLONG;
	//Check if file already exists in directory
	if (find_direntry(ospfs_inode(dir->i_ino), dentry->d_name.name, dentry->d_name.len) != NULL)
		return -EEXIST;
	
	//Check if entry cannot  be created, returns same error that ospfs_create returns
	ret_val = ospfs_create(dir, dentry, dir_oi->oi_mode, NULL);
	if (ret_val < 0)
		return ret_val;

	//get inode number of directory entry we are linking from
	entry_ino = find_direntry(ospfs_inode(dir->i_ino), 
		dentry->d_name.name, dentry->d_name.len)->od_ino;
	
	//Copy metadata to new link
	link_oi = (ospfs_symlink_inode_t*) ospfs_inode(entry_ino);

	link_oi->oi_size = symname_len;
	link_oi->oi_nlink = 1;
	link_oi->oi_ftype = OSPFS_FTYPE_SYMLINK;
	memcpy(link_oi->oi_symlink, symname, symname_len);
...
MODULE_AUTHOR("Chris Capistran & Amal Durisetti");
